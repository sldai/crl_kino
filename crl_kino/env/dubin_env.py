...
from crl_kino.utils.rigid import Obstacle, Rigid, CircleRobot, RectRobot, Vehicle
...
class DubinEnv(RobotEnv):
    def __init__(self, max_v=2.0, min_v=-0.1, max_phi=np.pi/3.0, env_bounds=[-20, 20, -20, 20], base_dt=0.1):
        super().__init__()
        ...
        self.rigid_robot = Vehicle(1.4, np.array([[-0.3, -0.5],[1.7,0.5]]),color='k')
    ...
    def motion_base(self, state, input_u, dt):
        x, y, theta = state
        vs, phi = input_u
        state[0] += vs*np.cos(theta)*dt
        state[1] += vs*np.sin(theta)*dt
        state[2] += vs/self.rigid_robot.d*np.tan(phi)*dt
        return state
    # def motion_base(self, state, input_u, dt):
    ...
    def get_bounds(self):
        state_bounds = np.zeros((3, 2))
        state_bounds[:2, :] = self.env_bounds
        state_bounds[2, 0] = -np.pi
        state_bounds[2, 1] = np.pi

        control_bounds = np.zeros((2, 2))
        control_bounds[0, 0] = self.min_v
        control_bounds[0, 1] = self.max_v
        control_bounds[1, 0] = self.min_phi
        control_bounds[1, 1] = self.max_phi
