...
class DDPGPolicy(BasePolicy):
    ...
    def forward(self, batch, state=None,
        ...
        if eps is None:
            eps = self._eps
        # shift the noise-adding process to the neural network
        logits, h = model(obs, eps=eps, state=state, info=batch.info)
        
        dev = logits.device
        low = torch.tensor(self._range[0], device=dev)
        high = torch.tensor(self._range[1], device=dev)

        # multidimensional clamp
        logits = torch.min(torch.max(logits, low.view((1,-1))), high.view((1,-1)))
