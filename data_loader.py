...
#Unseen_environments==> N=10, NP=2000,s=100, sp=0
#seen_environments==> N=100, NP=200,s=0, sp=4000
def load_test_dataset_no_cae(N=100,NP=200, s=0,sp=4000):

	obc=np.zeros((N,7,2),dtype=np.float32)
	temp=np.fromfile('obstacle/obs.dat')
	obs=temp.reshape((-1,2))

	temp=np.fromfile('obstacle/obs_perm2.dat',np.int32)
	perm=temp.reshape(77520,7)
