...
def main(args = get_args()):
    ...
    test_env.obc_list = obc_list[20:]

    suc_rate_train = np.zeros(run_num)
    time_train = np.zeros(run_num)
    for i in range(run_num):
        info, ctime, images = collect_data(train_env, policy)
        time_train[i] = ctime
        if info['goal']:
            suc_rate_train[i] = 1
            ...
            suc_rate_train[i] = 0
            imageio.mimsave(os.path.join('gif', 'train_collsion_'+str(i)), images, fps=10)
        else:
            suc_rate_train[i] = 0
            imageio.mimsave(os.path.join('gif', 'train_not_reach_'+str(i)), images, fps=10)
    suc_rate_train = np.size(suc_rate_train)/len(suc_rate_train)
    ...
    suc_rate_test = np.size(suc_rate_test)/len(suc_rate_test)

    print(suc_rate_train, suc_rate_test)

    plt.hist(time_train, label='train')
    plt.hist(time_test, label='test')
    plt.show()
