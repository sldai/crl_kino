...
def get_args():
    ...
    parser.add_argument("--case", type=str, default="collection")
...
def train(args=get_args()):
    ...
    assert len(dataset) == len(
        targets
    ), "Numbers of instances of inputs and outputs does not match"
    ...
    for epoch in range(args.epoch):
        ...
        for i in range(0, len(dataset), args.batch_size):
            ...
            i_ = (
                i + args.batch_size
                if i + args.batch_size <= len(dataset)
                else len(dataset)
            )
        ...
        if epoch != 0 and epoch % 50 == 0:
            ...
            if best_epoch == epoch:
                torch.save(
                    mlp.state_dict(), os.path.join(args.model_path, "estimator.pth")
                )
    writer.close()
